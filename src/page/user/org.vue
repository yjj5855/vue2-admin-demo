...
<script>
  export default{
    ...
    mounted () {
      ...
      var tree = d3.layout.tree().nodeSize([200, 80]);
      ...
      var zm = null;
      var svg = d3.select("#infovis").append("svg").attr("width", width).attr("height", height)
...
//          .attr("dy", ".35em")
      ...
      function autoWidth(d) {
        if(d.name && d.name.length > 10){
          return Math.max(240, rectW + (d.name.length - 10)*13)
        }
        return rectW
      }
      ...
      function autoX(d) {
        if(d.name && d.name.length > 10){
          return Math.min(-50, -((d.name.length - 10)*13) /2)
        }
        return 0
      }
